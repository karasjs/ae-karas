...
function recursion(data, library, newLib, w, h, start, duration, offset) {
  ...
  if(Array.isArray(anchorPoint) && anchorPoint.length) {
    res.animate.push(transformOrigin(anchorPoint, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(opacity) && opacity.length) {
    res.animate.push(transformOpacity(opacity, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(position) && position.length) {
    res.animate.push(transformPosition(position, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(rotateX) && rotateX.length) {
    res.animate.push(transformRotateX(rotateX, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(rotateX) && rotateX.length) {
    res.animate.push(transformRotateY(rotateY, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(rotateX) && rotateZ.length) {
    res.animate.push(transformRotateZ(rotateZ, start, duration, offset, startTime, inPoint, outPoint));
  }
  if(Array.isArray(scale) && scale.length) {
    res.animate.push(transformScale(scale, start, duration, offset, startTime, inPoint, outPoint));
  }
  return res;
}
